...
lruCache_device::lruCache_device(char * file_name, char * dataset_name, int maxElements, int3 cDim, int cI, int levelC, int nLevelsO, int maxElementsCPU)
{
...
lruCache_device::~lruCache_device()
{
...
void lruCache_device::changeDimensionCube(int maxElements, int3 cDim, int cI)
{
	delete queuePositions;
	cudaFree(cacheData);
	indexStored.clear();
...
void lruCache_device::updateCube(visibleCube_t * cube, int nLevels, int * nEinsertedCPU, int * nEinsertedGPU, cudaStream_t stream)
{
...
void lruCache_device::updateCache(visibleCube_t * visibleCubes, int num, int nLevels, cudaStream_t stream)
{
	int newCubesC = 0;
	int newCubesG = 0;
...
int	 lruCache_device::get_numElements(){return numElements;}
int3	 lruCache_device::get_cubeDim(){return cubeDim;}
int	 lruCache_device::get_cubeInc(){return cubeInc.x;}
void 	lruCache_device::printStatistics()
{ 
